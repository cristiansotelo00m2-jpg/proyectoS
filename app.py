...
# Configuración de base de datos dinámica con variables de entorno
_db_password = os.environ.get('DB_PASSWORD', 'AVNS_ODHeoLTFT_oJbSwVKam')
if _db_password == 'AVNS_0DHeoLTFT_oJbSwVKam':
    _db_password = 'AVNS_ODHeoLTFT_oJbSwVKam'
...
db_config = {
    ...
    'database': os.environ.get('DB_NAME', os.environ.get('DB_DATABASE', 'EcoGreen')),
    'ssl_disabled': os.environ.get('DB_SSL_DISABLED', 'false').lower() in ('true', '1', 'yes'),
}

def get_db_connection():
    """Función auxiliar para obtener la conexión a MySQL con soporte SSL."""
    try:
        ...
        try:
            ...
            ssl_ctx = None
            if not db_config.get('ssl_disabled', False):
            ...
            return PyMySQLWrapper(raw_conn)
        except Exception:
            raise err
